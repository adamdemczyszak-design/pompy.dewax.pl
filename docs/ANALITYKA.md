...
- **Mail ze zgłoszeniem.** Mechanizm „Źródło wejścia w zgłoszeniu” opisany wyżej: `js/dewax.js`
  zapamiętuje parametry kampanii z adresu wejścia (`dx_zrodlo`) i dopisuje je do zgłoszenia jako pole
  `zrodlo`; `wyslij.php` wpisuje je do maila jako „Źródło:” i osobno „Kreacja:” (wartość `utm_content`).
  Działa bez cookies, dla każdego zgłoszenia.
...
adresie wejścia kontaktu; wydatki per reklama z Meta (Windsor.ai).
