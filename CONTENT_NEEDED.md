...
| 10.8 | Luka po odrzuconych cookies (zgłoszenia, których HubSpot nie widzi) | Kod HubSpota ładuje się dopiero po zgodzie marketingowej, więc część zgłoszeń z formularza jest tylko w mailu. Pomiar: policzyć maile „Wycena: …” na `sprzedaz@dewax.pl` od 12.09.2026 i porównać z kontaktami z formularza `.zap` w HubSpocie (stan 13.09.2026 ok. 13:00: 6 kontaktów z kampanii, wszystkie z R3 „Kotłownia”). Skrzynka `sprzedaz@` nie jest dostępna z sesji Claude, więc liczbę maili podaje właściciel. Gdy luka przekracza mniej więcej jedną trzecią zgłoszeń: `wyslij.php` wysyła zgłoszenie do formularza HubSpot przez Forms API po stronie serwera (bez Make, bez sekretu). Tor przez Make z 12.09 został wycofany 13.09. |
...
| 10.11 | Druga aktywna kampania na koncie pomp | **Zrobione 13.09.2026** na polecenie właściciela: „Dewax gruntowe pompy ciepła \| Leads \| Image” (`120240091141860027`, 70 zł/dzień) wstrzymana. Kampania z 12.09 dostała na chwilę 130 zł/dzień, ale właściciel zdecydował: 60 zł/dzień, nie więcej (ustawione tego samego dnia). |
| 10.14 | Limit wydatków konta reklamowego | Konto `955522616312255` ma limit 16 763,50 zł, wydane 16 574,00 zł, zostało 189,50 zł (stan 13.09.2026). Przy 60 zł/dzień Meta zatrzyma wszystkie reklamy ok. 16.09. Podnieść limit w Menedżerze reklam → Ustawienia płatności → „Limit wydatków konta” (z sesji Claude nie da się tego zmienić). |
| 10.15 | Remarketing w Meta: grupy odbiorców | Z sesji Claude nie da się założyć grup odbiorców na tym koncie (Meta Ads MCP nie obsługuje konta, Windsor.ai nie ma takiej akcji). Właściciel zakłada w Menedżerze reklam trzy grupy z piksela (instrukcja: `reklamy/meta/KAMPANIA.md`, „Remarketing w Meta”) i podaje ich identyfikatory. Potem Claude zakłada zestaw B i reklamy `rm-1`, `rm-2` w tej samej kampanii, bez zwiększania budżetu 60 zł/dzień. |
| 10.16 | Grupa lookalike | Do założenia w Menedżerze reklam dopiero, gdy grupa źródłowa (`Lead` 180 dni albo `KalkulatorUkonczony` 30 dni) przekroczy 100 osób; dziś jest za mała. |
| 10.17 | Raport tygodniowy | **Nie da się założyć z sesji Claude**: harmonogram utworzony z sesji nie dostaje konektorów Windsor.ai i HubSpot (sprawdzone 13.09.2026, próbny harmonogram usunięty). Właściciel zakłada rutynę w claude.ai (Routines) z tymi konektorami, wklejając polecenie z `reklamy/generator/RAPORT-TYGODNIOWY.md`. Alternatywa: raz w tygodniu polecenie „zaktualizuj wyniki generatora” w sesji. |
...
| 10.13 | Kreacje z marką Buderus (`bud-1`, `bud-2`, `bud-3` w generatorze, 13.09.2026) | Teksty są gotowe, w rejestrze R1–R4, bez tytułu partnerskiego i bez parametrów urządzenia, bo tych nie da się potwierdzić z sesji Claude (buderus.com i portal partnerski są poza zasięgiem sieci sandboxa). Do dostarczenia przez właściciela do `reklamy/meta/buderus/`: (1) dokładny status partnerski (Autoryzowany Partner Handlowy, Top Partner albo Autoryzowany Instalator Pomp Ciepła) i model, który DEWAX montuje (np. Logatherm WSW196i.2); (2) zdjęcie produktu z portalu partnerskiego albo własne zdjęcie kotłowni z pompą Buderus (wyłącznie prawdziwe); (3) zasady użycia logo i nazwy Buderus dla partnerów (co-branding), jeśli logo ma być w reklamie; (4) czy w reklamie może paść „do 7 lat gwarancji u Autoryzowanych Partnerów Handlowych” (program Buderus; warunki do sprawdzenia w portalu). Bez punktów 1–2 kreacje zostają w bibliotece. |
